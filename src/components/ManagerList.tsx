...
export const ManagerList: React.FC<Props> = ({ managers, selectedManager, onSelect, timeRangeLabel = '今日' }) => {
  return (
          ...
          return (
            ...
            >
              ...
              <div className="text-sm text-gray-500 flex justify-between">
                ...
                <span>{timeRangeLabel}: {manager.todayVisitCount} 筆 {(manager.expectedStayMinutes || 0) > 0 && `(${manager.expectedStayMinutes}分)`}</span>
              </div>
              {(manager.visitedStores && manager.visitedStores.length > 0) && (
                <div className="text-xs text-gray-700 mt-2" style={{ marginTop: '8px', padding: '6px', backgroundColor: 'var(--color-secondary-50)', borderRadius: 'var(--radius-sm)' }}>
                  <strong>{timeRangeLabel}巡店：</strong>{manager.visitedStores.join(', ')}
                </div>
              )}
            </div>
          );
